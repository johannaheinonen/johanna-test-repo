...
## 1. Linux Packet Management (Debian based distributions)
Linux packet management installs, updates, configures and removes software on a linux. To do this root privileges are required as installing or removing software affects the system as a whole.  
...
```apt-cache search <package>```  
  - example:  
    ...
    ```apt-cache search tree | grep ^tree```  
      ...
      - | takes output from one command and feeds it as input into another
...
- /lib directiry for libraries   
- __/media__ is directory where external storage will be automatically mounted when you plug it in, for example /media/cdrom  
- /mnt directory is where you would manually mount storage devices or partitions  
- /opt directory for software that is not managed by system's package manager, for example software you build something from source code   
...
- cd .. (go to parent directory, move to one level up in the directory tree)
...
- absolute path: ```mkdir /home/testuser/new_project``` or ```mkdir ~/new_project``` creates a new directory ```new_project``` to ```/home/testuser/``` independent of the current working directory.
- relative path: ```mkdir new_project``` creates a new directory ```new_project``` to the current working directory. 

## 4. more or less?
```more``` and ```less``` are both used to view (long) text files or command output one screen at a time. ```more``` displays text page by page but only forward, ```less``` is more powerful allowing also backward navigation and search.  
...
- piping command output: ```apt-cache search tree |less```
...
- __nano__
- older ones: vi, emacs, pico (can be only a symbolic link to nano)
- __micro__ (homework exercise)
- vscode - for software development (multi-languags support, built-in git, GitHub copilot integration)

< nano editor picture here >
...
examples:  
	- ```mkdir ~/testdir1``` (This directory will be created under user home directory)  
 	- ```mkdir testdir2``` (This directory will be created under the directory where the user is located)  
  	- ```mkdir testdir3 testdir4``` (create multiple directories)  
    - ```mkdir -p testdir5/subdir/subsubdir``` (create nested directories, -p creates parent directories if they do not exist)  
	
### cp  
Cp copies file(s) or directories from `[source`] to `[destination`]  
examples:  
	- copy a file to another file: ```cp test.txt test-renamed.txt```    
	...
	- remove a file: ```rm file1.txt```  
 	- remove multiple files: ```rm file2.txt file3.txt``` (or use wildcards ```rm *.txt```)  
  	- interactive deletion: ```rm -i file2.txt```  
   	- recursive deletion - delete folder and all its contents: ```rm -r folder1/```  
...
### ln -s 
ln -s `[target`] to `[link_name`]  creates a symbolic link ('shortcut') between files and directories.  
...
```
example:  
	- create a symbolic link file.txt so that it can be accessed from user's home directory: ln -s ~/original/file.txt ~/linkedfile.txt  
...
```ls -l``` shows the file and directory permissions: the first character specifies directory(d) or file(-). User, group and others permissions (rwx) are following.  
```
