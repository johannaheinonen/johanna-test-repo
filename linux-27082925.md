...
When there is a need to upgrade installed packages to the latest versions (possibly adds new packages or removes obsolete ones that are required by updated dependencies):  
```sudo apt-get update```  
