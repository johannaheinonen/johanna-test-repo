...
```ls -l``` shows the file and directory permissions: the first character specifies directory(d) or file(-). User, group and others permissions (rwx) are specified after that. For example below ```file1.txt``` is a file (-) where user and group categories have read and write (rw) permissiona and others category only read (r) permission.  
