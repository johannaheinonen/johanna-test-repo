...
```cp -n``` does not overwrite existing files. If the destingation file exists nothing is copied.  
...
```mv -n``` does not overwrite existing files. If the destingation file exists nothing is moved.  
