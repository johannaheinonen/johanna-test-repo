...
- option1: in the terminal type: sudo __reboot__
...
Notice:
- modifying file that is not in your home directory:  ```sudo nano /etc/apt/sources.list```  or ```sudoedit /etc/apt/sources.list```  

How to copy-paste between host machine and virtualbox VM?  
	- shutdown your VM  
	- select your VM in Virtualbox manager  
 	- go to settings -> General -> Advanced menu  
  		- shared clippoad: bidirectional  
