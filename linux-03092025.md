...
Least privilege principle when configuring Apache2:   
 - ```sudo``` is used only when absolutely necessary
 - in this exercise the configuration changes will be done to files in ```*-available``` and ```*-enabled```. There is no need to modify e.g. apache.conf  
 - home pages will be created in user's home directory as a regular user -  ```sudo``` is not needed and should not be used  
...
ufw comes with predefined application profiles for Apache2:  
```sudo ufw app list```  
...
Available applications:
  ...
  Apache HTTP
```
To allow both HTTP (port 80) and HTTPS (port 443):  
```sudo ufw allow 'Apache full'```  

or allow ports manually:  
```
...
you should see these ports allowed: 
```
To                         Action      From
...
A name-based virtual host makes it possible to run multiple websites (e.g. ```site1.com``` and ```site2.com```) on a single server and IP-address. DNS resolves all sites to the same IP address. If real DNS is not available ```/etc/hosts``` can be used to map hostnames to IP addresses. For example:
```
127.0.0.1    site1.com
