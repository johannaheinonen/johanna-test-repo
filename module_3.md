...
The required steps are the following:  
 ...
 2. Create document root to your home directory: ```mkdir -p /home/testuser/public-sites/```.
 ...
 3. Set permissions:  
    ...
    if missing:  ```chmod o+x /home/testuser```  
...
Apache2 writes logs in two different locations, each serving a different purpose:
- __Systemd journal__ (system logging). Apache2 is managed by systemd, which is the system and service manager used by most modern Linux distributions.
Systemd is the first process that starts when Linux boots, and it is responsible for launching and supervising services like Apache. Because Apache2 runs as a systemd service, its service‑level messages (start, stop, failures, warnings) are stored in the systemd journal.
- __Apache’s own log files__. Apache also maintains its application‑specific logs inside ```/var/log/apache2/```. These include ```access.log``` that includes every HTTP request received and ```error.log``` that includes errors encountered while serving requests. These logs are essential for debugging websites, VirtualHosts, permissions, and missing files.

### Systemd log: journal
Systemd logs can be seen with the command ```journalctl```:  
- ```journalctl -u apache2``` shows all logs from the apache2 systemd service.  
- ```journalctl -f``` follows the journal in real time, useful for watching events e.g. while you restart or test a service.  
- ```journalctl -u apache2 -n 50```  shows the last 50 log lines from the apache2 service, useful if you only want the recent messages, not the whole log.

Notice: grep and pipe are very useful when doing troubleshooting with logs: ```journalctl -u apache2 -n 100 | grep -i error```

The systemd journal stores service‑related messages, not website traffic. This makes it useful for checking whether Apache2 has started correctly or if the service has crashed. The journal typically shows messages such as:  
- Apache start, stop or restart events  
- Configuration syntax errors  
- Module loading errors  
- Port conflicts  
- Permission issues  
- Any message apache sends to standard output (stdout) or standard error (stderr)  

You should check the journal for example when:  
- ```sudo systemctl restart apache2``` fails  
- if you edited files in ```/etc/apache2/sites-available/*.conf``` and Apache refuses to start or reload.  

### Apache access & error logs
Apache2’s access and error logs are stored in ```/var/log/apache2/```. These are the actual web server logs generated by Apache itself — not by systemd. These logs show what visitors are requesting, which URLs are being accessed and what kind of errors occur while serving pages. In other words, these logs tell you how the website is behaving and whether any requests are failing or producing errors. These logs can be seen with commands  
```tail -f /var/log/apache2/error.log```  
```tail -f /var/log/apache2/access.log```  

__access.log__ shows all incoming HTTP requests (client IP, URL, method, response code).  
__error.log__ shows errors while serving requests, for example:  
- missing files  
- wrong DocumentRoot  
- permission denied  
- VirtualHost configuration issues

__mysite-access.log__ and __mysite-error.log__ are virtualhost specific logs (optional).  

Check the Apache logs in /var/log/apache2/ whenever Apache2 is running but something on your website is not working correctly. These logs are especially useful if you experience issues such as:  
- VirtualHost configuration problems  
- your site showing the default Apache page instead of your own site  
- Empty pages or unexpected HTTP errors, such as:  
    - 404 Not Found  
    - 403 Forbidden  
