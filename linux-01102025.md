...
__Notice2__: environment variable ```$PATH``` is usually __not__ changed. As an exception user specific ```$HOME/bin/``` may be added:  
   ...
   To make this permanent it has to be added into ```.bashrc``` file. ```.bashrc``` is a hidden configuration file in user home directory that is used to initialize settings when a new terminal session is opened.  
   ```sudo cp .bashrc .bashrc.orig```  
   ```echo 'export PATH="$PATH:$HOME/bin"' >> ~/.bashrc```  
   ```source ~/.bashrc    # reload the .bashrc file into the current shell session without restarting the terminal```   
