...
When there is a need to upgrade installed packages to the latest versions:    
```sudo apt-get update```  
```sudo apt-get dist-upgrade```  (possibly adds new packages or removes obsolete ones that are required by updated dependencies)
...
- ```ls -la```  
- ```ls /```
- ```ls -F``` (adds a symbol at the end of each listed item, for example / is directory, * is executable file etc.)
- ```ls -lh``` (human readable file sizes like KB or MB)  
...
```cp -n``` does not overwrite existing files. If the destination file exists nothing is copied.  
...
```mv -n``` does not overwrite existing files. If the destination file exists nothing is moved.  
...
```ls -l``` shows the file and directory permissions: the first character specifies directory(d) or file(-). User, group and others permissions (rwx) are specified after that. For example below ```file1.txt``` is a file (-) where user and group categories have read and write (rw) permissions and others only read (r) permission.  
...
Examples - symbolic mode:  
	- ```script1.sh``` is a program that needs to be executed. It requires exec rights (x).  
...
### 7. Other useful commands
	- clear
 	- echo hello  (displays text to the display)  
